...
def get_crime_stats(zipcode):
	print('Looking up crime statistics for zipcode {}.'.format(zipcode))
	zip_count=defaultdict(int)
	surveyData = csv.reader(open("data\last_crime_nyc.csv"), delimiter=",")
	...
	for i in range(0,list_len):
		...
		if rslt == str(zipcode):
			print('Found crime stat ranking of {} for zipcode {}.'.format(zip_count[str(rslt)], zipcode))
			return zip_count[str(rslt)]
	
	print('Did not find crime stats for zipcode {}.'.format(zipcode))
	return 0
